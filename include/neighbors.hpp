...
template <class Metric> typename Metric::index_type
NearestNeighbors<Metric>::radius_neighbors(const Index *queries, Index num_queries, Real radius, RealVector& dists, IndexVector& neighs, IndexVector& ptrs, int num_threads) const
{
    std::vector<const Atom*> query_ptrs(num_queries);

    for (Index i = 0; i < num_queries; ++i)
        query_ptrs[i] = metric[queries[i]];
