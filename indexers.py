...
from extensions.cover_tree import cover_tree_euclidean, cover_tree_manhattan
...
class CoverTree(object):
    ...
    def __init__(self, points, metric, cover=1.3, leaf_size=30, num_threads=1):
        ...
        if self.metric == "euclidean":
            self.tree = cover_tree_euclidean(self.points)
            self.tree.build_index(self.points, cover, leaf_size, num_threads)
        elif self.metric == "manhattan":
            self.tree = cover_tree_manhattan(self.points)
            self.tree.build_index(self.points, cover, leaf_size, num_threads)
