...
void CoverTree<Distance>::build(const Real *points, Index n, int d, Real cover, int leaf_size)
{
    this->n = n;
    this->d = d;
    ...
    struct Hub
    {
        ...
        void compute_child_hubs(const Real *points, int dim, Real cover, Index leaf_size, Real maxdist)
        {
            Real sep;
            Index m = size();
            ...
            do
            {
                ...
                const Real *new_candidate_point = &points[new_candidate*dim];
...
template <class Distance>
Index CoverTree<Distance>::range_query(const Real *points, const Real *query, Real radius, IndexVector& neighbors, RealVector& dists) const
{
    Index neighs = 0;
